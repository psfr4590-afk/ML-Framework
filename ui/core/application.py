...
class Application(tk.Tk):
    ...
    def _style(self):
        """Configure Tk themed widgets while remaining usable on minimal Tk installs."""
        try:
            from tkinter import ttk
            style = ttk.Style(self)
            try: style.theme_use("clam")
            except tk.TclError: pass
            style.configure("ModelLab.TButton", padding=(10, 6), font=("Segoe UI", 9, "bold"))
            style.configure("ModelLab.TLabel", background=BG, foreground=TEXT)
            style.configure("ModelLab.TFrame", background=BG)
        except Exception:
            return
