...
def stage(did,name): return start_stage(did,name)
def status(did=None): return store.refresh_pipeline_state(did) if did else [store.refresh_pipeline_state(d["id"]) for d in store.list()]
...
def crawl_stats(did): return store.crawl_stats(did)
def crawl_domains(did): return store.crawl_domains(did)
def crawl_log(did,tail=80): return store.crawl_log(did,tail)
