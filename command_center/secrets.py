...
# Explicit allowlist: credentials may only populate variables consumed by the
# supported data providers. Arbitrary process-environment mutation is unsafe.
...
class CredentialStore:
    ...
    def set(self, name, secret, provider="custom", kind="token", env_var="", description="", identity=""):
        ...
        with LOCK:
            ...
            data["credentials"][name] = {
                "secret": base64.b64encode(fernet.encrypt(secret.encode())).decode(),
                "provider": provider,
                "type": kind,
                "env_var": env_var,
            ...
            self._save(data)
            return {"name": name, "provider": provider, "type": kind, "env_var": env_var,
                    "description": description, "identity": identity, "stored": True}

    def reveal(self, name):
        data = self._load()
        if name not in data.get("credentials", {}):
            raise KeyError(name)
    ...
    def list(self):
        ...
        for name, item in sorted(data.get("credentials", {}).items()):
            ...
            out.append({"name": name, "provider": item.get("provider", "custom"), "type": item.get("type", "token"),
                        "env_var": env_var, "description": item.get("description", ""),
                        "identity": item.get("identity", ""), "stored": True,
                        "environment_set": bool(env_var and os.environ.get(env_var))})
        return out
    ...
    def test(self, name):
        ...
        return {"ok": bool(value), "name": name}
