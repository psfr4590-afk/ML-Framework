...
class DatasetStore:
    ...
    def refresh_stats(self, did):
        """Compute dataset statistics from the latest valid pipeline corpus."""
        root = self.path(did)
        ...
        try:
            for dirpath, dirnames, filenames in os.walk(root):
                ...
                for filename in filenames:
                    ...
                    try: bytes_ += os.path.getsize(filepath)
                    except OSError: pass
        except OSError: pass

        # Prefer the latest canonical corpus, but include crawl-only sessions.
        # This keeps dataset.json useful immediately after crawl instead of
        # reporting zero documents until a later transformation exists.
        candidates = (
            root / "scratch" / "04_weighted.jsonl",
            root / "scratch" / "03_deduped.jsonl",
            root / "scratch" / "02_cleaned.jsonl",
            root / "scratch" / "01_crawled.jsonl",
        )
        for c in candidates:
        ...
        if stage == "shard":
            ...
            try:
                ...
                for item in files:
                    ...
                    try:
                        if shard_path.stat().st_size != int(item["size"]): return "corrupt"
                    except OSError: return "corrupt"
