...
def _load_checkpoint(path: Path) -> tuple[dict[str, torch.Tensor], ModelConfig, dict[str, Any]]:
    try:
        ...
        raise RuntimeError(f"Unable to read checkpoint {path}: {exc}") from exc
...
def _convert_to_f16(converter: Path, hf_dir: Path, gguf_dir: Path) -> Path:
    gguf_dir.mkdir(parents=True, exist_ok=True); out = gguf_dir / "model-f16.gguf"
...
def export_checkpoint(output_dir: str | Path, llamacpp_dir: str | Path, quant: str = "Q4_K_M", model_name: str = "pretrain-model", checkpoint: str | Path | None = None) -> dict[str, Any]:
    ...
    hf_dir = output_dir / "export" / "hf"; gguf_dir = output_dir / "gguf"
    _write_hf_checkpoint(state, cfg, hf_dir, model_name); _tokenizer_files(tokenizer_dir, hf_dir)
    converter = _find_converter(llamacpp_dir); f16 = _convert_to_f16(converter, hf_dir, gguf_dir); final = f16
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", required=True); parser.add_argument("--llamacpp-dir", required=True); parser.add_argument("--quant", default="Q4_K_M", choices=sorted(QUANTS)); parser.add_argument("--model-name", default="pretrain-model"); parser.add_argument("--checkpoint"); parser.add_argument("--verbose", action="store_true")
    args = parser.parse_args(argv); logging.basicConfig(level=logging.DEBUG if args.verbose else logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
