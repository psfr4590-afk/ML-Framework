...
def _native_smoke() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="model-lab-release-") as tmp:
        ...
        if not manifest_path.is_file(): print(f"Missing export manifest: {manifest_path}"); return 2
        try:
            manifest = json.loads(manifest_path.read_text(encoding="utf-8")); final_gguf = Path(manifest["final_gguf"])
            if not final_gguf.is_file() or final_gguf.stat().st_size <= 0: print(f"Missing final GGUF artifact: {final_gguf}"); return 2
            for key in ("dataset_card", "model_card"):
                card = Path(manifest[key])
                if not card.is_file() or card.stat().st_size <= 0: print(f"Missing export card: {key}: {card}"); return 2
                if manifest.get(f"{key}_sha256") != _sha256(card): print(f"Export card hash mismatch: {key}: {card}"); return 2
        except (OSError, ValueError, KeyError, TypeError) as exc: print(f"Invalid export manifest: {exc}"); return 2
        if run([sys.executable, "scripts/verify_gguf.py", "--model", str(final_gguf), "--llamacpp-dir", str(ROOT / "third_party" / "llama.cpp"), "--manifest", str(manifest_path)]): return 2
        try:
            result = json.loads(manifest_path.read_text(encoding="utf-8")).get("inference_validation", {})
            if result.get("status") != "passed" or not result.get("generated_output"): print("Export manifest does not contain a passing inference validation result"); return 2
        except (OSError, ValueError, TypeError) as exc: print(f"Unable to read validated export manifest: {exc}"); return 2
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the Model Lab production verification gate."); parser.add_argument("--bootstrap-native", action="store_true", help="bootstrap pinned llama.cpp and validate native GGUF inference"); parser.add_argument("--skip-tests", action="store_true", help="not allowed for production verification"); args = parser.parse_args()
    ...
    if _native_smoke(): print("RELEASE VERIFICATION FAILED: native export/inference"); print("Platform:", platform.platform()); return 2
