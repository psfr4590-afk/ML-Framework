...
SECTION_KEYS = {
    ...
    "stages": STAGES,
    "crawl": {"max_workers", "request_timeout", "retry_attempts", "retry_backoff", "respect_robots_txt", "politeness_delay", "max_pages_per_domain", "max_content_bytes", "max_redirects", "fail_on_malformed_jsonl", "dataset_groups_file", "dataset_profiles_file", "source_weights_file", "sources", "web", "github", "arxiv", "huggingface", "google"},
    "clean": {"config_file"},
...
def _strict_nested(name: str, value: dict) -> None:
    ...
    if unknown:
        raise ValueError(f"Unsupported {name} configuration keys: {', '.join(sorted(unknown))}")
...
def validate_config(cfg: dict) -> None:
    ...
    for name in ("sources", "web", "github", "arxiv", "huggingface", "google"):
        ...
        if value is not None:
            if not isinstance(value, dict):
                raise ValueError(f"crawl.{name} must be an object")
    ...
    for key in ("train_on_sample",):
        if key in tok:
            _bool("tokenizer", key, tok[key])
    if "shuffle_docs" in shard:
        _bool("shard", "shuffle_docs", shard["shuffle_docs"])
    for key in ("resume", "allow_cpu_training", "auto_size"):
        if key in train:
            _bool("train", key, train[key])
